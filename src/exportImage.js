...
(function exportImage() {
  // download img
  function triggerDownload(imgURI, title = "") {
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    if (firstChild.nodeType === Node.ELEMENT_NODE) {
      var isResponse = ele.classList.contains("font-claude-message");
      var childNodes = firstChild.childNodes;

      // Prefix Claude reponse label
      if (isResponse) {
        imageContent += responseStartDiv;
        imageContent += divWrapper("<em>Claude:</em>");
      } else {
  ...
  footerDiv.innerText = "Generated with ryanschiang/claude-export";
